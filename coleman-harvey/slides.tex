...
\begin{frame}{The big picture}
    ...
    {\tiny Arul, Balakrishnan, Best, Bradshaw, Castryk, Costa, Denef, Gaudry, Gurel, Harvey, Kedlaya, Magner, Minzlaff, Shieh, Triantafillou, Tuitman, Vercauteren, and more\ldots}
...
\begin{frame}
    ...
    \begin{alertblock}{Stumbling block}
        This is no longer linear in the index!
        You cannot apply BGS to this recurrence.
    \end{alertblock}
    \pause%
...
\begin{frame}
    ...
    \resizebox{\linewidth}{!}{%
        $\displaystyle
            \kbordermatrix{
                \amp x^{t-2g}\diff x/y          \amp \cdots \amp x^t \diff x /y                   \amp        \amp f(P)\\
                x^{t-2g}\diff x/y \amp (2t - 1)r_{0,0} + 2s'_{0,0}\amp \cdots \amp (2t - 1)r_{2g-1,0} + 2s'_{2g-1,0}\amp \vrule \amp     \\
                \vdots \amp \vdots\amp\ddots\amp\vdots\amp  \vrule \amp   \\
                x^{t-2g}\diff x/y \amp (2t - 1)r_{0,2g-1} + 2s'_{0,2g-1} \amp \cdots \amp (2t - 1)r_{2g-1,2g-1} + 2s'_{2g-1,2g-1}\amp  \vrule \amp \\
                \cline{2-6} % -------------------------------------------------
    ...
    so that we keep in memory a vector \(v\in W_{t}\times \QQ_p\).
...
\begin{frame}{Many integrals simultaneously}
    ...
    \resizebox{\linewidth}{!}{%
        $\displaystyle
        \kbordermatrix{
            \amp x^{t-2g}\diff x/y          \amp \cdots \amp x^t \diff x /y                   \amp        \amp f(P_1)\amp \cdots\amp f(P_L)\\
            x^{t-2g}\diff x/y \amp (2t - 1)r_{0,0} + 2s'_{0,0}\amp \cdots \amp (2t - 1)r_{2g-1,0} + 2s'_{2g-1,0}\amp \vrule \amp     \\
            \vdots \amp \vdots\amp\ddots\amp\vdots\amp  \vrule \amp   \\
            x^{t-2g}\diff x/y \amp (2t - 1)r_{0,2g-1} + 2s'_{0,2g-1} \amp \cdots \amp (2t - 1)r_{2g-1,2g-1} + 2s'_{2g-1,2g-1}\amp  \vrule \amp \\
            \cline{2-8} % -------------------------------------------------
    ...
    \begin{block}{Note}
        This matrix and products of it have the same fixed form, when running BGS we don't try and interpolate entries that are always 0 \(\leadsto\) better run time.
    \end{block}
\end{frame}
